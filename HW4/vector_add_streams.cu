...
int main() {
    ...
    int blocks_half = (half + threadsPerBlock - 1) / threadsPerBlock;
    vector_add<<<blocks_half, threadsPerBlock, 0, stream1>>>(d_A, d_B, d_C, half);
    vector_add<<<blocks_half, threadsPerBlock, 0, stream2>>>(d_A + half, d_B + half, d_C + half, half);
